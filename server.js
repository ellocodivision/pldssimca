...
const VICEROY_RESERVATION_STATIC_LAYOUT = {
  page1: {
    ...
    paymentLabelRows: [
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 99, 334, 170, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 99, 350, 170, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 99, 366, 170, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 99, 382, 170, 14)
    ],
    paymentPercentRows: [
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 248, 323, 28, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 248, 339, 28, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 248, 355, 28, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 248, 371, 28, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 248, 387, 28, 14)
    ],
    paymentAmountRows: [
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 300, 327, 92, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 300, 343, 92, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 300, 359, 92, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 300, 375, 92, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 300, 391, 92, 14)
    ],
    paymentDateRows: [
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 452, 325, 134, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 452, 341, 134, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 452, 357, 134, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 452, 373, 134, 14),
      rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 452, 389, 134, 14)
    ],
    amountCurrency: rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 52, 423, 260, 18),
    cardNumber: rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 329, 423, 260, 18),
    cardName: rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 52, 453, 260, 18),
    cardType: rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 329, 453, 260, 18),
    cardAddress: rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 52, 485, 260, 18),
    cardCityStateZip: rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 52, 517, 540, 18),
    expirationDate: rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 52, 549, 260, 18),
    securityCode: rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 329, 549, 260, 18),
    signature: rectFromTop(VICEROY_RESERVATION_PAGE_HEIGHT, 210, 624, 200, 18)
  },
  page2: {
...
const VICEROY_RESERVATION_LAYOUT_PATH = path.join(DATA_DIR, 'viceroy-reservation-layout.json');
...
function normalizeWhisperlistKpi(raw) {
  ...
  return {
    ...
    contratoEnProcesoDeCambios: normalizeYesNo(item.contratoEnProcesoDeCambios) || 'NO',
...
function normalizeWhisperlistRow(rawRow, fallbackId) {
  ...
  return {
    ...
    kpi: normalizeWhisperlistKpi({
      ...
      contratoEnProcesoDeCambios: normalized.contrato_en_proceso_de_cambios || normalized.kpi_contrato_en_proceso_de_cambios,
...
function getViceroyPresentationFloorImageSource(floor) {
  const src = floor && typeof floor === 'object' ? floor : {};
  return normalizeViceroyPresentationUrl(
    String(src.imageDataUrl || src.imageRawDataUrl || src.imageUrl || src.floorImageUrl || src.planImageUrl || '').trim()
  );
}
...
function readViceroyPresentationFloorData() {
  ...
  if ((!Array.isArray(floorsData.floors) || !floorsData.floors.length) && requestedName) {
    floorsData = readMergedFloorsByDevelopment('viceroy-piloto');
  }
  return floorsData;
}
...
function buildViceroyPresentationInventoryStatusMap() {
  ...
  for (const candidate of candidates) {
    try {
      ...
      log(`No se pudo leer inventario Viceroy para presentación desde ${candidate.fullPath}: ${err && err.message ? err.message : err}`);
    }
  }
  if (!rows.length) {
    const cached = readViceroyInventoryCache();
...
function drawTextInRect(page, font, rect, text, options = {}) {
  ...
  const x = options.align === 'center'
    ? rect.x + Math.max(marginX, (rect.width - textWidth) / 2)
    : (options.align === 'right'
      ? rect.x + Math.max(marginX, rect.width - textWidth - marginX)
      : rect.x + marginX);
...
function reservationBackdropColor() {
...
function readViceroyReservationLayout() {
  return normalizeViceroyReservationLayout(readJson(VICEROY_RESERVATION_LAYOUT_PATH, null));
}

function saveViceroyReservationLayout(layout) {
  ...
  try {
    ...
    const nextRow = {
      ...
      kpi: normalizeWhisperlistKpi({
        ...
        contratoEnProcesoDeCambios: body.contratoEnProcesoDeCambios !== undefined ? body.contratoEnProcesoDeCambios : target.kpi && target.kpi.contratoEnProcesoDeCambios,
...
app.post('/api/viceroy/kpi-reservas/rows', requireViceroyPresentAccess, async (req, res) => {
  try {
    ...
    const newRow = {
      ...
      tipoVenta: normalizeWhisperlistTipoVenta(body.tipoVenta),
      nombreCliente,
      unidadReservada: normalizeWhisperlistPersonText(body.unidadReservada),
      documentosCompletos: normalizeYesNo(body.documentosCompletos) || 'NO',
      recamaras: '',
      pais: normalizeWhisperlistCountry(body.pais),
      ciudad: normalizeWhisperlistCity(body.ciudad),
      clientEmail: normalizeClientEmail(body.clientEmail),
      ...
      kpi: normalizeWhisperlistKpi({
        ...
        contratoEnProcesoDeCambios: body.contratoEnProcesoDeCambios,
  ...
  try {
    ...
    if (index >= 0) {
      ...
      nextRow = {
        ...
        tipoVenta: body.tipoVenta !== undefined ? normalizeWhisperlistTipoVenta(body.tipoVenta) : normalizeWhisperlistTipoVenta(whisperRow.tipoVenta),
        ...
        unidadReservada: body.unidadReservada !== undefined ? normalizeWhisperlistPersonText(body.unidadReservada) : normalizeWhisperlistPersonText(whisperRow.unidadReservada),
        documentosCompletos: body.documentosCompletos !== undefined ? normalizeYesNo(body.documentosCompletos) : normalizeYesNo(whisperRow.documentosCompletos) || 'NO',
        ...
        kpi: normalizeWhisperlistKpi({
          ...
          contratoEnProcesoDeCambios: body.contratoEnProcesoDeCambios !== undefined ? body.contratoEnProcesoDeCambios : whisperRow.kpi && whisperRow.kpi.contratoEnProcesoDeCambios,
...
app.get('/api/viceroy/inicio/reservation-layout', requireBackendModule('viceroy'), (req, res) => {
  ...
  return res.json({
    ok: true,
    layout: readViceroyReservationLayout(),
    isGerente: Boolean(currentEmail && currentEmail === GERENTE_EMAIL)
  });
});

app.post('/api/viceroy/inicio/reservation-layout', requireGerente, (req, res) => {
  try {
    const incoming = req.body && req.body.layout ? req.body.layout : req.body;
    const saved = saveViceroyReservationLayout({
      ...incoming,
      updatedBy: String(req.user && req.user.email || '').trim().toLowerCase()
    });
    return res.json({ ok: true, layout: saved });
  } catch (err) {
    return res.status(500).json({
      error: 'No se pudo guardar el layout de la hoja de reserva de Viceroy',
      details: err && err.message ? err.message : 'error desconocido'
    });
  }
});
...
function normalizeViceroyKpiReservasVisibleRow(row, currentEmail, currentName, isGerente, userMap) {
  ...
  const visibleRow = {
    ...
    kpi: {
      ...
      contratoEnProcesoDeCambios: normalizeYesNo(visibleKpi.contratoEnProcesoDeCambios) || 'NO',
...
function viceroyKpiReservasExportRows(rows, userMap) {
  ...
  return items.map((row) => ({
    ...
    CONTRATO_EN_PROCESO_DE_CAMBIOS: normalizeYesNo(row && row.kpi && row.kpi.contratoEnProcesoDeCambios),
